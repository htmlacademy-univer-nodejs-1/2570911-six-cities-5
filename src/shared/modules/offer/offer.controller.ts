...
import { CheckHostMiddleware } from '../../libs/rest/middleware/check-host.middleware.js';
...
export class OfferController extends BaseController {
  constructor(
    ...
    this.addRoute({
      ...
      middlewares: [
        ...
        new ValidateObjectIdMiddleware('offerId'),
        new DocumentExistsMiddleware(this.offerService, 'Offer', 'offerId'),
        new CheckHostMiddleware(this.offerService, 'offerId'),
  ...
  public async getFavorites(req: Request, res: Response): Promise<void> {
    const { id: userId } = req.tokenPayload!;
    const offers = await this.offerService.getUserFavorites(userId);
    this.ok(res, fillDTO(FullOfferRdo, offers));
  }
  ...
  public async addToFavorites(req: Request, res: Response): Promise<void> {
    const { offerId } = req.params;
    const { id: userId } = req.tokenPayload!;
    const offer = await this.offerService.addFavorite(offerId, userId);
    this.ok(res, fillDTO(OfferRdo, offer));
  }
  ...
  public async removeFromFavorites(req: Request, res: Response): Promise<void> {
    const { offerId } = req.params;
    const { id: userId } = req.tokenPayload!;
    const offer = await this.offerService.removeFavorite(offerId, userId);
    this.ok(res, fillDTO(OfferRdo, offer));
  }
}
